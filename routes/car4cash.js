...
router.get("/", async (req, res) => {
  try {
    ...
    try {
      await page.waitForSelector("#onetrust-accept-btn-handler", { timeout: 250 });
    ...
    await page.waitForFunction(() => {
      return document.querySelector(".multiselect__single")?.innerText.includes("สินเชื่อรถยนต์");
    });
    await delay(250);

    // 📌 4️⃣ คลิก dropdown "ยี่ห้อรถ"
    await page.waitForSelector('div[data-test="brand-dropdown"] .multiselect__select');
    ...
    const brands = await page.evaluate(() => {
      let options = Array.from(document.querySelectorAll('div[data-test="brand-dropdown"] li.multiselect__element .option-box span'));
    ...
    const models = await page.evaluate(() => {
      let options = Array.from(document.querySelectorAll('ul.multiselect__content li.multiselect__element .option-box span'));
      return options.map(option => option.innerText.trim());
    });
